const myLeaves = async (page) => {
  const data = await fetch(`/api/v2/user/leaves?page=${page}`);
  const { leaves, filteredUsersCount, resultPerPage, leavesCount } =
    await data.json();
  return { leaves, filteredUsersCount, resultPerPage, leavesCount };
};
...
const renderLeavesDetails = async (page = 1) => {
  ...
  const { leaves, resultPerPage } = await myLeaves(page);
  if (page) {
...
const setPagination = async (page = 1) => {
  ...
  const { filteredUsersCount, resultPerPage, leavesCount } = await myLeaves();
