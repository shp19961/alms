...
const newLeaveRequests = async (page) => {
  const data = await fetch(
    `/api/v2/reimbursement/admin/leave/request?page=${page}`
  );
  const {
    reimbursement,
    resultPerPage,
...
const setHeaders = async () => {
  ...
  if (userr.role === "admin") {
    ...
    // </li>
    // <li class="nav-item">
    //   <a class="nav-link" href="/new">New Employee</a>
    // </li>
    // <li class="nav-item">
    //   <a class="nav-link" href="/archive">Archive Employee</a>
    // </li>
    // <li class="nav-item">
    //   <a class="nav-link" href="/allusers">All Employee</a>
    // </li>
    // <li class="nav-item">
    //   <a class="nav-link" href="/userdetails">Employee Details</a>
    // </li>
    // <li class="nav-item">
    //   <a class="nav-link" href="/reqleave">Update Leaves</a>
    // </li>
    // <li class="nav-item">
    //   <a class="nav-link" href="/oldreqleave">Old Leaves</a>
    // </li>
    // <li class="nav-item">
    //   <a class="nav-link" href="/reqreimbursement">Comp-Off</a>
    // </li>`;
  }
...
const userRequest = async (page = 1) => {
  ...
  const { reimbursement: reqData, resultPerPage } = await newLeaveRequests(
    page
  );
  ...
  for (i = 0; i < reqData.length; i++) {
    if (reqData[i].status === "confirm") {
      contentDiv.innerHTML += `<tr class="column_of_table">
        ...
        <td>${reqData[i].holidayDate}</td>
        <td>${reqData[i].leaveTakenDate}</td>
        <td>
          <div class="button-group">
          <button id="${
            reqData[i]._id
          }" class="acceptToggleColor req-button" value="confirm" disabled><span>&#10003;</span></</button>
          <button id="${
            reqData[i]._id
    ...
    item.addEventListener("click", async () => {
      ...
      if (!item.classList.contains("editBtn")) {
        const res = await fetch(
          "api/v2/reimbursement/leave/request/update",
          requestOptions
        );
